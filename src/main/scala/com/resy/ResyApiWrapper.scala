...
object ResyApiWrapper {
  ...
  private val apiMap: Map[ResyApiMapKey, ApiDetails] = Map(
    ...
    BookReservation ->
      ApiDetails(
        ...
        testFailureResponse =
          """{"message": "Invalid user authentication credentials were provided."}"""
      )
  )
  ...
  def sendGetRequest(resyApiMapKey: ResyApiMapKey, queryParams: Map[String, String])(implicit
    ...
    val apiDetails = apiMap(resyApiMapKey)
    val url =
      s"https://${apiDetails.url}?${stringifyQueryParams(queryParams)}"
  ...
  private[this] case class ApiDetails(
    url: String,
    testSuccessResponse: String,
    testFailureResponse: String
  )
}
